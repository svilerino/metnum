...
class SlowMotionEffect {
	...
	private:
		...
		static void spline_method_interpolation(interpolation_method_t interp_method, uint interpol_frame_count, Video& video_input, Video& video_output);
		...
		static void spline_algorithm(vector<pixel_t>& x0, vector<pixel_t>& y0, polinomio_conjunto& pol_interpolate);
		static cubicas_matriz interpolar_bloque(Video& video_input, uint punto_origen, uint punto_destino);
};

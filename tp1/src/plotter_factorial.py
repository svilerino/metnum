...
if(graphictype <=4):
	...
	for line in dataFile:
		...
		if(ultima_cant_nodos != nodos):
			if(ultima_cant_nodos != -1):						
				...
				graphic_med_y_over_n.append(float(value / math.pow(ultima_cant_nodos, 6)))
				graphic_med_y_over_n_square.append(float(value / math.pow(ultima_cant_nodos, 7)))
				graphic_med_y_over_n_cube.append(float(value / math.pow(ultima_cant_nodos, 8)))

				factorialnumber = Decimal(math.factorial(ultima_cant_nodos))
				value = Decimal(value)
				graphic_med_y_over_n_fourth.append(Decimal(value / factorialnumber))
				acum = 0
				cant_densidades_por_cant_nodos = 0
	...
	#print graphic_med_y_over_n_fourth
